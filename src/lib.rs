...
impl Write for LoggerBackend {
    ...
    fn write(&mut self, message: &[u8]) -> io::Result<usize> {
        match self {
            &mut LoggerBackend::Unix(ref dgram) => dgram.send(&message[..]),
            &mut LoggerBackend::UnixStream(ref mut socket) => {
                let null = [0; 1];
                socket.write(&message[..]).and_then(|_| socket.write(&null))
            }
            &mut LoggerBackend::Udp(ref socket, ref addr) => socket.send_to(&message[..], addr),
            &mut LoggerBackend::Tcp(ref mut socket) => socket.write(&message[..]),
        }
    }

    fn flush(&mut self) -> io::Result<()> {
        match self {
            &mut LoggerBackend::Unix(_) => Ok(()),
            &mut LoggerBackend::UnixStream(ref mut socket) => socket.flush(),
            &mut LoggerBackend::Udp(_, _) => Ok(()),
            &mut LoggerBackend::Tcp(ref mut socket) => socket.flush(),
        }
    }
}
...
pub fn init(
    ...
    let backend = unix(formatter.clone())
        ...
        .or_else(|_| TcpStream::connect(("127.0.0.1", 601)).map(|s| LoggerBackend::Tcp(s)))
        .or_else(|_| {
